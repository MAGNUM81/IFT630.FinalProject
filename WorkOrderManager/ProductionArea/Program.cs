...
namespace ProductionArea
{
	internal class Program
	{
		...
		private static ProductionAreaServer server;
		private static BackupManager backupManager;
		private static ProductionAreaManager prodManager;
		...
		private static void Main(string[] args)
		{
			...
			{
				prodManager =
					new ProductionAreaManager(
						ref server); //This runs on the main thread... should it though? like. everything else has its own thread so...
				Console.WriteLine("HELLO! PRODUCTION AREA MANAGER UP AND RUNNING.");
			}
			catch (Exception e)
			{
				Console.WriteLine(
					"Something went wrong initializing and/or running the Production Area Manager. We must abort the mission.");
				Console.WriteLine(e.Message);
				AbortInit();
			}
			...
			{
				Console.WriteLine(
					"Something went wrong while running the Production Area Manager. We must abort the mission.");
				Console.WriteLine(e.Message);
				AbortInit();
			}
		}
